...
- **Single pass** — `JSON.stringify` creates the exact phase-1 source once; phase 2 walks the parsed value once to build the full line index, max line length, and global non-punctuation token table. The provisional painter scans only its bounded prefix; after hydration, Text and Find use the global table. Tree is LAZY: `flatten()` rebuilds it on demand (first Tree open / worker `getTree`), same philosophy as lazy labels/min. No second value walk or pretty rebuild on the paste path, no re-stringify on toggle.
...
- **Punct tokens dropped by design** — braces/colons/commas render via base `code` color, which equals the punct color. The global token-pair contract therefore compares against `tokenize(pretty)` **filtered to non-punct**. Re-adding them re-bloats the table.
...
`bun run build` (tsc + singlefile build), `bun run test`, `bun tests/fuzz.ts`, `bun run bench` — bench gates the PASTE COMPUTE PATH via the **walk/native ratio** (per-run: (total − parse − stringify) / (parse + stringify), min-of-7, trip at 0.75): absolute ms flake with machine heat/load (observed 21→40ms with zero path changes), but native floor and pipeline drift TOGETHER, so only real walk regressions move the ratio. The CLI excludes structured-clone/postMessage, worker task hops, main-thread delivery, rAF scheduling, and hydration; its first/full values are compute-ready estimates, not browser paint/full-ready measurements. Tree/min are lazy and reported on-demand, not gated. `bun scripts/bench-viewport.ts` is a deterministic historical token-cost comparison; production labels there are HTML compute, not browser paint. A feature ships only with the gate green; a perf change ships only with the bench number attached.
