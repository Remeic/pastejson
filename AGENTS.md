...
- **Punct tokens dropped by design** — braces/colons/commas render via base `code` color, which equals the punct color. Window tables therefore compare token pairs against `tokenize(pretty)` **filtered to non-punct**. Re-adding them re-bloats the table.
