...
- **Tree walk is closure-free too, with preview-canonical interning** — `flatten()` follows the same doctrine as the paste walk: one closure-free body, pooled frames held in a `cf` register, top-level (non-capturing) `internLeaf`. Leaf interning is canonical: plain **no-escape short** strings key on the raw value (repeats never re-quote), numbers key on the number (no `String()` on repeats), literals key on their **own** map (a JSON string `"true"` must not collide with boolean `true`), and every other string keys on the **preview string** — so preview-sharing (escaped-short preview vs long truncated preview) still dedupes exactly like the old single-map walk. `vals` must stay UNIQUE: splitting the key space wrongly duplicates entries and shifts `valIdx`. Measured ~2× over the old closure/class walk on the 5MB payload (27ms → 10ms; `bun run bench` lazy line) and faster on every shape tested.
- **Punct tokens dropped by design** — `rangeHtml` paints each token from the PREVIOUS token's end, so an inter-token gap (whitespace + punct) inherits the FOLLOWING token's class; punctuation after the last token uses base `code` color. Pretty drops punct in `emitJson`; Min drops it too — `buildMinTokens` calls `tokenize(min, true)` and `tokM` is punct-free. So Min punctuation colors exactly like Text. Fuzz therefore compares token pairs against `tokenize(pretty)` **filtered to non-punct**. Re-adding them re-bloats the table (Min pays −56% more tokens).
...
- **Scroll space is capped and compressed** — browsers cap element scroll height (~33.5M px Chrome, ~17.9M Firefox); past a 10M px floor `VScroll` maps scrollTop→row over the whole `[0, N-V]` range so rows beyond the cap stay reachable (else search/scroll silently can never reach a deep match). Search jumps call `scroller.scrollToRow(row)`, which centres the row and ALWAYS repaints — `paintNow`'s window dedupe would otherwise skip a current-match flip when the window is byte-identical (`scripts/verify-search-nav.sh` guards all of this).
...
- `flatten()` columns resolve byte-identical (rowCount, depth/kind/meta/subtreeRows, `keyIdx`→`keys[]`, `valIdx`→`vals[]`) to the single preview-keyed reference walk, and `vals` is UNIQUE. `tests/run.ts` covers literal/string/number and short/long preview-boundary collisions.
- Worker replies: fixed key order, buffers transferred, never HTML strings.

## Red gate

`bun run build` (tsc + singlefile build), `bun run test`, `bun tests/fuzz.ts`, `bun run bench` — bench gates the PASTE PATH via the **walk/native ratio** (per-run: (total − parse − stringify) / (parse + stringify), min-of-7, trip at 0.75): absolute ms flake with machine heat/load (observed 21→40ms with zero path changes), but native floor and pipeline drift TOGETHER, so only real walk regressions move the ratio. Tree/min are lazy and reported on-demand, not gated (their bench line is a single cold sample — verify perf with an interleaved min-of-N harness, not that line). A feature ships only with the gate green; a perf change ships only with the bench number attached.
