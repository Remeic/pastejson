...
pastejson ([json.justgiulio.dev](https://json.justgiulio.dev/)) is a free, single-purpose JSON tool: paste JSON, see it formatted instantly. It handles multi-megabyte documents in milliseconds because it is built directly on the browser's native JSON parser with a hand-tuned rendering pipeline — no frameworks on the hot path, no network round-trips, no server.
